...
namespace GeneFinder.Viewmodels
{
    public class SmorfsList
    {
        ...
        public string IsIntergenic { get; set; }
        ...
        public string ExonInFrame { get; set; }
        public float GCcontent { get; set; }
        public float Conservation5UTR { get; set; }
        public float Conservation3UTR { get; set; }
        public float ConservationsmORF { get; set; }
        public string ConservationString { get; set; }
        public string RNAMatch { get; set; }
        public string ProteomicMatch { get; set; }
        ...
        public SmorfsList(smorf s)
        {
            ...
            GCcontent = s.GCcontent1;
            Conservation5UTR = s.ConservationPrevious;
            Conservation3UTR = s.ConservationPosterior;
            ConservationString = s.ConservationString;
            RNAMatch = "";
            ProteomicMatch = "";
            ...
            if (s.condensedResultTable == null)
            {
                ...
                Introns = 0;
                Ambiguous = 0;
                IsIntergenic = (TranscriptNumber == 0).ToString();
                ExonInFrame = false.ToString();
            }
            else
            {
                ...
                for (int i = 0; i < s.annotationResult.Count; i++)
                {
                    for (int j = 0; j < s.annotationResult[i].Count; j++)
                    {
                        ...
                        {
                            ExonInFrame = true.ToString();
                            break;
                        }
                    }
                    if (ExonInFrame == true.ToString()) break;
                }
            }
